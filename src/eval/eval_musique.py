...
def load_hotpotqa(dataset_path=None, n=20):
    """Loads PRE-PROCESSED MuSiQue dataset and formats it for the evaluator."""
    if not dataset_path or not Path(dataset_path).exists():
        raise FileNotFoundError(f"Could not find MuSiQue dataset at {dataset_path}. Run preprocess_hotpotqa_retrieval.py first.")
        
    print(f"Loading local pre-processed MuSiQue dataset from {dataset_path}...")
...
def run(dataset_path, n):
    print(f"\nLoading MuSiQue dataset (n={n})...")
    dataset = load_hotpotqa(dataset_path, n=n) 
    print(f"Loaded {len(dataset)} samples\n")

    output_dir = Path(project_root) / "eval_results" / "musique"
    ...
    print("\n✓ Master results saved to eval_results/musique/final_benchmark_results.csv")

def parse_args():
    parser = argparse.ArgumentParser(description="Run MuSiQue Generative Benchmark")
    parser.add_argument(
        "-n", "--num_samples", 
        type=int, 
...
if __name__ == "__main__":
    args = parse_args()
    run(dataset_path="data/musique/musique_top30_hybrid_500.json", n=args.num_samples)